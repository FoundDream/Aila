...
export async function loadSkillFromDir(directory: string): Promise<LoadedSkill> {
  ...
  return { directory: resolved, skillPath, definition, body }
}
...
function renderSkillInvocation(skill: LoadedSkill, bundledFiles: readonly string[]): string {
  const lines = [
    ...
    tools: [
      {
        ...
        run: async (args) => {
          ...
          // Re-read from disk so SKILL.md edits apply without an extension
          // reload; fall back to the loaded snapshot if the re-read fails.
          let current = skill
          try {
            current = await loadSkillFromDir(skill.directory)
          } catch {
            current = skill
          }
          return renderSkillInvocation(current, await listSkillBundledFiles(current.directory))
        },
      },
    ],
  }
