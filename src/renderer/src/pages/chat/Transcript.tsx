  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-1 items-center justify-center">
        <p className="text-[22px] font-medium text-[var(--text)]">
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-8 py-8">
        <div
          ref={contentRef}
          className="mx-auto flex max-w-[680px] flex-col gap-7"
        >
          {messages.map((message) => (
            <MessageRow key={message.id} message={message} />
          ))}
          {canRetryLast && onRetryLast && (
            <RetryLastTurn onRetryLast={onRetryLast} />
          )}
        </div>
      </div>
      {!isAtBottom && (
...
function messageToPlainText(message: Message): string {
  return message.blocks
    .filter(
      (b): b is Extract<Block, { type: "text" | "reasoning" }> =>
        b.type === "text",
    )
    .map((b) => b.content)
    .join("\n\n")
  ...
  if (block.type === "image") {
    return (
      <figure className="flex flex-col gap-1">
        ...
        />
        {block.prompt && (
          <figcaption className="text-[11.5px] text-[var(--text-dim)]">
            {block.prompt}
          </figcaption>
        )}
      </figure>
...
function ToolStatusIcon({
  ...
  if (status === "error") {
    return (
      <AlertCircleIcon className="size-3.5 shrink-0 text-[var(--error)]" />
    );
  }
...
function ToolPayload({
  ...
  return (
    <div className="mb-2 last:mb-0">
      <div className="mb-1 text-[11px] font-medium text-[var(--text-dim)]">
        {label}
      </div>
      <pre
...
function toolActionLabel(name: string): string {
  ...
  if (["bash", "shell", "run_shell", "exec_command"].includes(normalized))
    return "Ran command";
  if (["read", "read_file", "open_file"].includes(normalized))
    return "Read file";
  if (["write", "write_file", "create_file"].includes(normalized))
    return "Wrote file";
  if (["edit", "apply_patch", "replace"].includes(normalized))
    return "Edited file";
  ...
  if (["generate_image", "image_generation"].includes(normalized))
