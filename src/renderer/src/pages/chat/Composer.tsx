        ...
        <div className="flex items-end gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3 transition-colors focus-within:border-[var(--border-strong)]">
          ...
          {isStreaming ? (
            ...
            <button
              ...
              onClick={() => void submit()}
              disabled={!canSend}
              className="h-7 shrink-0 rounded-md bg-[var(--brand-ink)] px-3 text-xs font-medium text-[var(--brand-ink-fg)] transition-colors disabled:cursor-not-allowed disabled:bg-[var(--surface-hover)] disabled:text-[var(--text-dim)]"
            >
              Send
            </button>
          )}
